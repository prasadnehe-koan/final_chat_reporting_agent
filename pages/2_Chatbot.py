...
# Example: LOGO_URL = "https://yourcompany.com/logo.png"
...
#     st.markdown(f"""
#     <div class="user-info">
#         👤 Logged in as: <strong>{username}</strong> • 💬 {msg_count} message(s) in current chat
#     </div>
#     """, unsafe_allow_html=True)
...
#             with col1:
#                 button_label = f"{'📌 ' if is_active else '💬 '}{chat_data['title']} ({msg_count})"
#                 if st.button(button_label, key=f"chat_{chat_id}", use_container_width=True, disabled=is_active):
#                     switch_chat(chat_id)
#                     st.rerun()
...
#     total_messages = sum(len(chat['messages']) for chat in st.session_state[session_key].values())
#     st.markdown(f"""
#     <div style="font-size: 0.75rem; color: var(--secondary-text-color); text-align: center; padding: 0.5rem; margin-top: 0.5rem;">
#         📊 {len(st.session_state[session_key])} chat(s) • {total_messages} total messages<br>
#         👤 {username} (ID: {user_id})
#     </div>
#     """, unsafe_allow_html=True)
...
#     <div style="flex: 1;">
#         <h1 style="color: white; font-size: 1.5rem; font-weight: 700; margin: 0;">💬 {current_chat['title']}</h1>
#         <p style="color: rgba(255,255,255,0.95); font-size: 0.875rem; margin: 0.25rem 0 0 0;">
#             Context maintained • {len(current_chat['messages'])} messages in this chat
#         </p>
#     </div>
#     <img src="{LOGO_URL}" style="height: 60px; width: auto; max-width: 200px;" alt="Logo">
# </div>
# """, unsafe_allow_html=True)
...
#         ✅ <strong>Active Context:</strong> This chat has {len(current_chat['messages'])} messages. 
#         Only messages from THIS chat are sent to the AI (isolated context).
#     </div>
#     """, unsafe_allow_html=True)
# else:
#     st.markdown(f"""
#     <div class="chat-context-info">
#         🆕 <strong>New Context:</strong> Start a fresh conversation. 
...
# with chat_container:
#     if not current_chat["messages"]:
#         st.info("👋 Start a conversation by typing a message below! Your chat context will be saved automatically.")
#     else:
#         for msg in current_chat["messages"]:
#             with st.chat_message(msg["role"]):
#                 st.markdown(msg["content"])
...
# st.markdown("""
# <div style="text-align: center; font-size: 0.8rem; padding: 0.75rem; color: var(--secondary-text-color);">
#     Powered by Koantek • 🔒 Full context persistence across sessions • User-wise & Chat-wise isolation
# </div>
# """, unsafe_allow_html=True)
...
#         cursor.execute("""
#             INSERT OR REPLACE INTO user_chats (chat_id, user_id, title, created_at, is_current)
#             VALUES (?, ?, ?, ?, ?)
#         """, (chat_id, user_id, title, created_at.isoformat(), 1 if is_current else 0))
...
#         cursor.execute("UPDATE user_chats SET is_current = 0 WHERE user_id = ?", (user_id,))
#         cursor.execute(
#             "UPDATE user_chats SET is_current = 1 WHERE chat_id = ? AND user_id = ?", 
#             (chat_id, user_id)
#         )
#         conn.commit()
...
# if session_key not in st.session_state:
#     # Load from database for this user
#     loaded_chats, loaded_chat_id = load_chats_from_db(user_id)
...
# def get_current_chat():
#     """Get the current active chat"""
#     return st.session_state[session_key].get(st.session_state.current_chat_id, {
#         "title": "New Chat",
#         "messages": [],
#         "created_at": datetime.now()
#     })
...
#         if chat_id == st.session_state.current_chat_id:
#             if st.session_state[session_key]:
#                 st.session_state.current_chat_id = list(st.session_state[session_key].keys())[0]
#                 set_current_chat_db(user_id, st.session_state.current_chat_id)
#             else:
#                 create_new_chat()
...
#     /* Rest of your existing chatbot styles... */
# </style>
# """, unsafe_allow_html=True)
...
#     st.markdown(f"""
#     <div style="font-size: 0.75rem; color: var(--secondary-text-color); text-align: center; padding: 0.5rem; margin-top: 0.5rem;">
#         📊 {len(st.session_state[session_key])} chat(s)<br>
#         👤 {username}
#     </div>
#     """, unsafe_allow_html=True)
...
# st.markdown("""
# <div style="text-align: center; font-size: 0.8rem; padding: 0.75rem; color: var(--secondary-text-color);">
#     Powered by Koantek • User-specific chat history
# </div>
# """, unsafe_allow_html=True)