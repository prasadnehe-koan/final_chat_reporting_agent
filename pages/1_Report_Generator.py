...
JOB_ID              = st.secrets.get('JOB_ID')           # ← Required for serverless (saved job ID)
...
# MODERN CSS STYLING
# ==========================================================
# (Your full CSS remains unchanged – keeping it exactly the same)
st.markdown("""
<style>
    /* Global Styles */
...
# JOB SUBMISSION & MONITORING LOGIC (UPDATED FOR SERVERLESS)
if run_btn:
    if not report_query.strip():
        st.warning("⚠️ Please enter a query to generate a report.")
    elif not all([DATABRICKS_TOKEN, DATABRICKS_INSTANCE, NOTEBOOK_PATH, JOB_ID]):
        st.error("🔧 Configuration Error: Please check your Databricks settings (JOB_ID is required for serverless).")
    # Prevent duplicate submissions
        ...
        # Use Jobs API 2.2 / run-now with saved job (serverless by default)
        payload = {
            "job_id": int(JOB_ID),
            "notebook_params": {
                "user_question": report_query,
                "user_id": str(user_id),
                "username": username
            }
        }
        
        run_url = f"{DATABRICKS_INSTANCE}/api/2.2/jobs/run-now"
        
        with st.spinner("🔄 Submitting job to Databricks (Serverless)..."):
            try:
                res = requests.post(run_url, headers=headers, json=payload, timeout=30)
                            ...
                            try:
                                ...
                                if file_res.status_code == 200:
                                    ...
                                    st.download_button(
                                        ...
                                        key=f"download_{idx}_{hash(file_path)}",
                                        use_container_width=True
                                    )
                                else:
...
<div style="text-align: center; font-size: 0.8rem; padding: 0.75rem;">
    <p class="footer-text">Powered by Koantek • Logged in as: {username}</p>
</div>
''', unsafe_allow_html=True)
